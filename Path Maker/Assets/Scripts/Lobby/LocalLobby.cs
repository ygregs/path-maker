...
namespace PathMaker
{
    ...
    public class LocalLobby : Observed<LocalLobby>
    {
        ...
        public void AddPlayer(LobbyUser user)
        {
            ...
            {
                Debug.LogError($"Cant add player {user.DisplayName}{(user.ID)} to lobbby: {LobbyID} twice");
                return;
            }
        ...
        private void DoRemoveUser(LobbyUser user)
        {
            if (!m_LobbyUsers.ContainsKey(user.ID))
            {
                Debug.LogError($"Player {user.DisplayName}{(user.ID)} does not exist in lobby: {LobbyID}");
                return;
            }
