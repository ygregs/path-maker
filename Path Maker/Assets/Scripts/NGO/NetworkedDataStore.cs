...
namespace PathMaker.ngo
{
    ...
    public class NetworkedDataStore : NetworkBehaviour
    {
        ...
        public void AddPlayer(ulong id, string name, TeamState teamState)
        {
            if (!IsServer)
                return;

            if (!m_playerData.ContainsKey(id))
                m_playerData.Add(id, new PlayerData(name, id, 0, teamState));
            else
                m_playerData[id] = new PlayerData(name, id, 0, teamState);
        }
